...
hideAllButton.addEventListener("click", () => {
  for (const key in userMarkers) {
    // Tạm thời không cập nhật vịtrí của tất cả users
...
map.on("click", (e) => {
  // Kiểm tra nếu đang dùng tool drawing line thì không tạo marker
  if (drawing.getMode() === "draw_line_string") return;
  if (drawing.getMode() === "draw_polygon") return;  
  if (drawing.getMode() === "draw_point") return;
